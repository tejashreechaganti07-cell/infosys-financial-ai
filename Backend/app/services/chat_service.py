...
from app.schemas.chat import ChatQueryRequest, ChatMessageResponse, ChatQueryResponse, Citation, ChatHistoryResponse

class ChatService:
    @staticmethod
    async def query_chat(user_id: str, query_in: ChatQueryRequest) -> ChatQueryResponse:
        db = get_db()
        ...
        # Generate rich placeholder reasoned financial answer
    ...
    @staticmethod
    async def get_history(user_id: str, workspace_id: str) -> ChatHistoryResponse:
        db = get_db()
        chat_col = db["chat_history"]
