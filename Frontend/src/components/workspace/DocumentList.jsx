...
export const DocumentList = ({ documents = [], onDeleteSuccess }) => {
  const handleDelete = async (id, title) => {
    if (
      !window.confirm(
        `Are you sure you want to delete "${title}" from vector DB?`
      )
    ) {
      return;
    }
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h4
                      className="font-semibold text-slate-200 text-xs truncate max-w-xs"
                      title={doc.title}
                    >
                      {doc.title}
                    </h4>
                  ...
                  <p className="text-[11px] text-slate-400 font-mono mt-0.5">
                    {doc.company_name} • {doc.filing_type} (FY
                    {doc.fiscal_year}) • {doc.chunks_count} chunks indexed
                  </p>
                </div>
              </div>
